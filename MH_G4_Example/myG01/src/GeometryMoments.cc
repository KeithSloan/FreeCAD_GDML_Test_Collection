...
std::vector<Moments> getMoments(G4VPhysicalVolume *pv, G4AffineTransform parentTransform) {
  ...
  if (lvmother != nullptr) {
	...
	if (calculator != nullptr) {
	  Moments momc = calculator->calculate();
	  getWorldMoments(pv, momc, parentTransform);
	  prettyPrint(pv->GetName(), momc.Volume, momc.cm, momc.M, solid->GetEntityType());
	  moments.push_back(momc);
	  delete calculator;
	}
	else {  
