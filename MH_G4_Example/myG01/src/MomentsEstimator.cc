...
MomentsEstimator* MomentsEstimator::getEstimator(G4VSolid *solid) {
  ...
  if (type == "G4MultiUnion") {
	// G4cout << "found multiUnion" << G4endl;
	return new MultiUnionMomentsEstimator(solid);
  }
  else {
	return new MomentsEstimator(solid);
  }
}
...
Moments MomentsEstimator::estimate()
{
  ...
  // G4cout << "MC Volume (cm3): " << MCVolume/cm3 << "  GetCubicVolume (cm3): " << moments.Volume/cm3 << G4endl; 
